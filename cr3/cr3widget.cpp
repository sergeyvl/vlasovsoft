...
CR3View::CR3View( QWidget *parent)
    ...
    , pDict(new Dictionary(this, NULL))
{
...
bool CR3View::loadDocument(const QString& fileName)
{
    ...
    {
        // check this document is already opened
        CRFileHist* hist = _docview->getHistory();
        if (    hist
             && !hist->getRecords().empty()
             && cr2qt(hist->getRecords()[0]->getFilePathName()) == fileName )
        {
            return false;
        }
    }
...
int getBatteryState()
{
    return Platform::get()->getBatteryLevel();
}
...
void CR3View::paintEvent( QPaintEvent * event)
{
    ...
    if ( _editMode )
    {
        ...
        {
            if ( cursorRc.left<0 )      cursorRc.left = 0;
            if ( cursorRc.top<0 )       cursorRc.top = 0;
            if ( cursorRc.right>dx )    cursorRc.right = dx;
            if ( cursorRc.bottom > dy ) cursorRc.bottom = dy;
...
bool CR3View::isTranslationMode() const
{
    return pDict->isVisible();
}

void CR3View::onTranslateStart()
{
    pDict->moveDown();
    pDict->raise();
    pDict->show();
...
void CR3View::onTranslateTap( const QPoint& pos )
{
    lvPoint pt (pos.x(), pos.y());
    ...
    if ( p.isText() )
    {
        ...
        if ( pos.y() <= height()/2 )
            ...
            pDict->moveUp();
        pDict->translate( _selText );
    }
}

void CR3View::onTranslateFinish()
{
    pDict->hide();
}
